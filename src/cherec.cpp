...
bool Cherec::init()
{
    ...
    // Reads the cherec mesh from a file, which contains a list of vertices and
    // indices
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint16_t) * m_indices.size(),
    ...
    // Loading shaders
    if (!effect.load_from_file(shader_path("salmon.vs.glsl"),
            shader_path("salmon.fs.glsl")))
        return false;
...
void Cherec::draw(const mat3& projection)
{
    transform.begin();
    ...
    glVertexAttribPointer(in_position_loc, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
        (void*)0);
    glVertexAttribPointer(in_color_loc, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
        (void*)sizeof(vec3));
...
// Cherec::collides_with(const Turtle& turtle)
// {
...
// bool Cherec::collides_with(const Fish& fish)
// {
...
// cherec - wall collisions.
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

vec2 Cherec::get_position() const
{
    return motion.position;
}

void Cherec::move(vec2 off)
{
    motion.position.x += off.x;
    motion.position.y += off.y;
}

void Cherec::set_rotation(float radians)
{
    motion.radians = radians;
}

bool Cherec::is_alive() const
{
    return m_is_alive;
}

// Called when the cherec collides with a turtle
void Cherec::kill()
{
    m_is_alive = false;
}

// Called when the cherec collides with a fish
void Cherec::light_up()
{

...
std::vector<Dot> Path::form(vec2 start, vec2 self_box, vec2 end, std::vector<Wall> &m_walls)
{
	int current_steps = 0;
	m_dots.clear();
	...
	while (!(len({end.x - idx + dx, end.y - idy + dy}) < 70))
	{
		...
		{
			// fprintf(stderr, "2");
			// vec2 pos = wall.get_position();
			// vec2 box = {wall.get_bounding_box().x + 150.f, wall.get_bounding_box().y + 150.f};
			// while (wall.collides_with_point(idx + dx, idy + dy, 3.f))
			// {
			// 	// fprintf(stderr, "3");
			// 	vec2 dest;
			// 	if (pos.x - box.x < idx + dx && pos.x + box.x > idx + dx && dy >= 0)
			// 	{
			// 		dest = {idx, idy + box.y * 1.5f};
			// 		dy = (dest.y - idy) / (steps - current_steps);
			// 		dx = (dest.x - idx) / (steps - current_steps);
			// 	}
			// 	else if (pos.x - box.x < idx + dx && pos.x + box.x > idx + dx && dy < 0)
			// 	{
			// 		dest = {idx, idy - box.y * 1.5f};
			// 		dy = (dest.y - idy) / (steps - current_steps);
			// 		dx = (dest.x - idx) / (steps - current_steps);
			// 	}
			// 	else if (pos.y - box.y < idy + dy && pos.y + box.y > idy + dy && dx < 0)
			// 	{
			// 		dest = {idx - box.x * 1.5f, idy};
			// 		dy = (dest.y - idy) / (steps - current_steps);
			// 		dx = (dest.x - idx) / (steps - current_steps);
			// 	}
			// 	else if (pos.y - box.y < idy + dy && pos.y + box.y > idy + dy && dx > 0)
			...
			// 		dx = (dest.x - idx) / (steps - current_steps);
			// 	}
			// 	while (!(len({dest.x - idx + dx, dest.y - idy + dy}) < 200))
			// 	{
			// 		// fprintf(stderr, "4");
			// 		idx += dx;
			// 		idy += dy;
		...
		dy = (end.y - idy) / (steps - current_steps);
		dx = (end.x - idx) / (steps - current_steps);
		idx += dx;
		idy += dy;
		Dot d = {true, {idx, idy}, 3.f};
		current_steps++;

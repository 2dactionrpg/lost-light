...
void PhysicsSystem::sync(entt::registry &registry, float ms, vector<Wall> &walls)
{
    ...
    for (auto enemy : viewEnemy)
    {
        ...
        auto &[id, health, enemy_type, is_alive, is_movable, attack_cooldown, attack_frequency, destination, target, es] = viewEnemy.get<enemyComponent>(enemy);
        ...
        float step = speed * (ms / 1000);
        if (is_alive && is_movable && (enemy_type == BOSS || enemy_type == ZOMBIE || es.action == MOVE))
        {
            if (direction.x > 0)
            {
...
float PhysicsSystem::update(entt::registry &registry, Character &m_character, Shield &m_shield, vector<Enemy> &m_enemies, vector<Zombie> &m_zombies, vector<Projectile> &m_projectiles, Potion &m_potion, Ground &m_ground, Overlay& m_overlay)
{
    ...
    for (auto entity : enemy)
    {
        ...
        auto &[id, health, enemy_type, is_alive, is_movable, attack_cooldown, attack_frequency, destination, target, es] = enemy.get<enemyComponent>(entity);
        auto &scale = enemy.get<physicsScaleComponent>(entity).scale;
        // just to stop extra iterations
        ...
        if (!found_flag)
        {
            for (auto &m_zombie : m_zombies)
            {
                if (id == m_zombie.get_id())
                {
